...
# The following 'trick' avoids this failing at synthesis time (while
# still working at implementation time), because at implementation
# time clocks from OOC IPs are not available yet. The downside is that
# one now, technically, misses those constraints during synthesis.
set pins [get_pins -hierarchical -filter {NAME =~infra/dma/xdma/*/phy_clk_i/bufg_gt_userclk/O}]
if {[llength ${pins}]} {
    create_generated_clock -quiet -name axi_clk ${pins}
...
# time clocks from OOC IPs are not available yet. The downside is that
# one now, technically, misses those constraints during synthesis.
set txout_clocks [get_clocks -filter {name =~ txoutclk*}]
if {[llength ${txout_clocks}]} {
    set_clock_groups -asynchronous -group [get_clocks osc_clk] -group [get_clocks -include_generated_clocks clk_aux] -group [get_clocks -include_generated_clocks ipbus_clk] -group [get_clocks -include_generated_clocks ${txout_clocks}]
} else {
    set_clock_groups -asynchronous -group [get_clocks osc_clk] -group [get_clocks -include_generated_clocks clk_aux] -group [get_clocks -include_generated_clocks ipbus_clk]
}
